...
class Training:
    ...
    def train(self,transformed_data,schema):
        ...
        #load model
        self.model = AutoModelForSequenceClassification.from_pretrained(self.training_config['model'],num_labels = self.training_config['num_labels'],id2label = schema['mapping']['int2str'], label2id =  schema['mapping']['str2int']).to("cuda")
        ...
        self.validation_result = pd.DataFrame({'label': preds.label_ids, 'prediction': np.argmax(preds.predictions,axis = 1),self.training_config['id_column']:transformed_data["test"][self.training_config['id_column']]})
    ...
    def save_model(self):
        ...
        checkpoints = [checkpoint for checkpoint in os.listdir(save_dir) if checkpoint.split("-")[0] == "checkpoint"]
        ...
        if len(checkpoints) == 1:
            ...
            os.rename(os.path.join(save_dir,best_checkpoint), os.path.join(save_dir,"best_model"))
