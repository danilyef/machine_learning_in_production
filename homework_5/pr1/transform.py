...
class Transform:
    ...
    def preprocess_fn(self, dataset):
        ...
        transformations = {
            'remove_stopwords': lambda batch: delete_stopwords(batch,self.stop_words_nltk),
        ...
        for step in transform_steps:
            ...
            dataset = dataset.map(transformation_fn, batched=True, batch_size = batch_size)


        # Train Tokenizer
        tokenizer_name = self.transform_config['tokenizer_info'].get('tokenizer_name') 
        tokenizer_training = bool(self.transform_config['tokenizer_info'].get('tokenizer_training'))
        ...
        variable_to_tokenize = self.transform_config['tokenizer_info'].get('variable_to_tokenize')
        ...
        # Tokenize
        tokenize = lambda example: self.tokenizer(example[variable_to_tokenize],truncation=truncation, max_length = max_length,return_tensors='pt',padding='max_length')
