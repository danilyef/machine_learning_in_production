...
def main():
    ...
    transform_config = read_json_file(
        os.path.join(pipeline_configs, "transform_config.json")
    )
    training_config = read_json_file(os.path.join(pipeline_configs, "training_config.json"))
    data_type_config = read_json_file(
        os.path.join(pipeline_configs, "data_type_config.json")
    )
    ...
    example_gen = ExampleGen(
        data_dir=data_dir, metadata_dir=metadata_dir, dataset_config=dataset_config
    )
    ...
    training = Training(
        ...
        tokenizer=transform_data.tokenizer,
