...
def load_inference_data(dataset: Output[Dataset]):
    ...
    test_dataset_obj = torchvision.datasets.MNIST(
        root=Path("./app/data"),
        train=False,
        download=True,
...
def run_inference(
    model: Input[Model], dataset: Input[Dataset], result: Output[Dataset]
):
    import torch
    import torchvision
    import shutil
    ...
    test_loader = torch.utils.data.DataLoader(
        test_dataset_obj, batch_size=64, shuffle=True
    )
...
def mnist_pipeline():
    ...
    save_model_task = save_inference_results(
        result=run_inference_task.outputs["result"]
    )
...
if __name__ == "__main__":
    ...
    pipeline_info = client.upload_pipeline(
        ...
        pipeline_name="MNIST Inference Pipeline",
